...
### 개요
- **미션:** VOD 편성 기획 전 과정에 AI 적용, 데이터 기반 의사결정 체계 구축
...
**미션:** AI 이미지 자동 생성 + 템플릿 대량 생산 (월 500~2,000건, 브랜드 가이드 준수)
...
**인증:** SSO + 자체 로그인, JWT(Access15분/Refresh7일), 2FA, 비밀번호 정책
...
git 사용법
1. 시작 : "git pull 하고 feature/task-name 브랜치만들어줘.
2. 구현 : TMDB API 연동 로직을 n8n 워크플로우에 맞게 수정해줘
3. 검증 : pytest 실행하고 에러 있으면 수정해줘
4. 커밋 : 변경 내역 요약해서 커밋해줘
5. 제출 : 현재 브랜치를 origin에 push해줘
