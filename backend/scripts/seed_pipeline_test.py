...
def seed_pipeline_test(db) -> dict:
    ...
    for title, year, synopsis, num_seasons, eps_per_season in COMPLETE_SERIES:
        series = Content(
            ...
            status=ContentStatus.approved, cp_name=CP,
            production_year=year, country="KR",
        )
        db.add(series)
        db.flush()
    ...
    for (title, stored_year, correct_year, stored_genre,
        ...
        db.add(ContentMetadata(
            ...
            ai_synopsis=f"{title} AI 처리 시놉시스",
            ai_genre_primary=stored_genre,   # 오류 장르 의도적 입력
            ai_mood_tags=["충돌테스트"],
            quality_score=68.0,
            ai_processed_at=_now(),
        ))
...
def clean_pipeline_test(db, dry_run: bool = False) -> int:
    ...
    for Model in [ContentAIResult, ExternalMetaSource, ContentImage,
                  ContentCredit, ContentMetadata]:
        db.query(Model).filter(Model.content_id.in_(ids)).delete(synchronize_session=False)

    # Content는 parent_id 자기참조 FK → 자식 먼저 삭제 (is_deleted=False 무관)
