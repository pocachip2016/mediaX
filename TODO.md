...
## Now (진행 중, 1~3개)
- [ ] **dev-auto-schedule Step 8** — conflict_service 검증(pytest 미실행, test_conflict_service.py 존치) + 통합 배선 재적용(auto_service _exec_p5_conflict 실구현/P6 발행 가드 + router GET /auto/sets/{id}/conflicts + schemas ConflictItemOut/ConflictReportOut — Steps1-7 클린 커밋 위해 revert됨) + FE 충돌 패널(ExposureCalendar 재사용) + verify auto-schedule-s8
