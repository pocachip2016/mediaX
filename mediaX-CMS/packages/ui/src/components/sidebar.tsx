...
function Sidebar({
  ...
  // 사이드바 너비를 state에 따라 직접 계산 (Tailwind v4 CSS변수 셀렉터 우회)
  const gapWidth =
    state === "collapsed"
      ? collapsible === "offcanvas"
        ? "0px"
  ...
  return (
    <div
      ...
      data-slot="sidebar"
    >
      {/* Gap div — inline style로 너비 직접 제어 */}
      <div
      ...
      <div
        ...
        data-side={side}
        style={{
          left: side === "left" ? containerLeft : undefined,
          right: side === "right" ? containerLeft : undefined,
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      className={cn(
        "absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear group-data-[side=left]:-right-4 sm:flex",
...
function SidebarGroupLabel({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      className={cn(
        "flex h-8 shrink-0 items-center rounded-md px-2 text-xs font-medium text-sidebar-foreground/50 transition-[margin,opacity] duration-200 ease-linear",
