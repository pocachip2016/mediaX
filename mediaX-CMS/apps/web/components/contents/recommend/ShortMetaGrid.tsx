...
export function ShortMetaGrid({ content, recommendations, appliedFields, onApply, inheritedFields }: Props) {
  return (
    <div className="bg-white rounded-lg border overflow-hidden">
      {/* 컬럼 헤더 */}
      <div className="grid grid-cols-[56px_1fr_1fr] border-b bg-slate-50">
      ...
      {FIELDS.map(({ field, label, icon }, idx) => {
        ...
        return (
          <div
            key={field}
            className={cn(
              "grid grid-cols-[56px_1fr_1fr]",
              idx > 0 && "border-t border-slate-100"
            )}
          >
            {/* 행 라벨 */}
            <div className="px-2 py-3 border-r border-slate-100 flex flex-col items-center justify-start pt-3 gap-0.5">
