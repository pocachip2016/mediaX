...
| **status** | **service** | waiting/processing/staging/approved/rejected — 편성 워크플로우 |
| **cp_name, cp_email_id** | **service** | CP사 관계 — 비즈니스 |
...
|---|---|---|---|
| CP 원본 메타 | cp_synopsis, cp_genre, cp_tags, cp_cast, cp_poster_url | **service** | 유지 (CP 워크플로우) |
| AI 생성 메타 | ai_synopsis, ai_genre_primary/secondary, ai_mood_tags, ai_cast, ai_rating_suggestion | **service** | 유지 (AI 처리 결과) |
...
| 확정 메타 | final_synopsis, final_genre, final_tags, final_cast, final_source | **service** | 유지 (검수 결과) |
| 품질 | quality_score, score_breakdown | **service** | 유지 (품질 기준) |
| 완료 플래그 | text/image/video_meta_completed | **service** | 유지 (워크플로우 플래그) |
| 기술 메타 | video_resolution/format, codec_*, bitrate, duration, subtitle_languages | **service** | 유지 (납품 파일 기술 정보) |
| DRM | drm_type | **service_kt → ContentDistribution** | step0 이후 ContentDistribution 으로 이동 예정 |
| 플랫폼 노출 | preview_clip_url | **service** | 유지 |
| 처리 이력 | ai_processed_at, reviewed_by, reviewed_at | **service** | 유지 |
...
poster/thumbnail/stillcut — TMDB·KOBIS 포스터는 meta_core 이미지. 자체 제작 이미지도 동일 테이블에 `source=manual` 로 보관.

---

### 8. service 전용 테이블 (이동 불필요, 레이블링만)

| 테이블 | Layer | 비고 |
|---|---|---|
| content_ai_results | service | AI 처리 이력 |
| content_batch_jobs | service | CP 배치 업로드 |
| cp_email_logs | service | CP 이메일 수신 |
...
| ContentDistribution | dev-service-distribution step0 | service |
